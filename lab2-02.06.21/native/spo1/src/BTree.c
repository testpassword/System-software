...
uint32_t ParseLeafNode(char *rawNode, const char *folderName, uint32_t folderParentId, BTHeaderRec btreeHeader,
    ...
    for (int i = 0; i < descriptor.numRecords; i++) {
        HFSPlusCatalogKey key = CAST_PTR_TO_TYPE(HFSPlusCatalogKey, (rawNode + recordAddress[i]));
        ...
        if (recordType == FolderRecord) {
            catalogFolder = CAST_PTR_TO_TYPE(HFSPlusCatalogFolder,(rawNode + recordAddress[i] + key.keyLength + sizeof(key.keyLength)));
            ConvertCatalogFolder(&catalogFolder);
            if (HFSStrToStrCmp(key.nodeName, folderName)) return catalogFolder.folderID;
        } else continue;
    }
    return 0;
}
...
void ParseLeafNodeContent(char *rawNode, uint32_t parentID, BTHeaderRec btreeHeader, BTNodeDescriptor descriptor, FileSystem *fs) {
    ...
    for (int i = 0; i < descriptor.numRecords; i++) {
        ...
        if (CheckForHFSPrivateDataNode(key)) continue;
...
void ListDirectoryContent(uint32_t parentID, BTHeaderRec catalogBTHeader, FileSystem *fs) {
    ...
    while (!isLastNode) {
        ...
        if (descriptor.fLink == 0) isLastNode = true;
...
HFSPlusCatalogFile * GetFileRecordFromLeafNode(uint32_t fileId, BTNodeDescriptor descriptor, BTHeaderRec btreeHeader, char *rawNode) {
    ...
    for (int i = 0; i < descriptor.numRecords; i++) {
        ...
        if (recordType == FileRecord) {
            *catalogFile = CAST_PTR_TO_TYPE(HFSPlusCatalogFile,(rawNode + recordAddress[i] + key.keyLength + sizeof(key.keyLength)));
            ConvertCatalogFile(catalogFile);
            if (catalogFile->fileID == fileId) return catalogFile;
            else continue;
        } else continue;
    }
    free(catalogFile);
...
uint32_t ParseLeafNodeWithCondition(char *rawNode, const char *folderName, uint32_t folderParentId, BTHeaderRec btreeHeader,
                                    BTNodeDescriptor descriptor, HFSDataRecordType recordTypeToFind) {
    uint16_t recordAddress[descriptor.numRecords];
    FillRecordAddress(btreeHeader, descriptor, rawNode, recordAddress);
    for (int i = 0; i < descriptor.numRecords; i++) {
        HFSPlusCatalogKey key;
        ...
        if (recordType == FolderRecord && recordTypeToFind == FolderRecord) {
            ...
            catalogFile = CAST_PTR_TO_TYPE(HFSPlusCatalogFile,(rawNode + recordAddress[i] + key.keyLength + sizeof(key.keyLength)));
            ConvertCatalogFile(&catalogFile);
            if (HFSStrToStrCmp(key.nodeName, folderName)) return catalogFile.fileID;
        } else continue;
    }
    return 0;
}
...
HFSPlusCatalogFile *GetFileRecord(uint32_t fileId, BTHeaderRec catalogBTHeader, FileSystem fs) {
    ...
    while (!isLastNode) {
        ...
        if (catalogFile != NULL) return catalogFile;
        GetNextBlockNum(&nodeBlockNumber, &extentNum, &currentBlockNum, descriptor, fs);
    }
    free(rawNode);
    return NULL;
